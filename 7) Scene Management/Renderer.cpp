...
Renderer::Renderer(Window& parent) : OGLRenderer(parent)
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		s->SetTransform(Matrix4::Translation(Vector3(0, 100.0f, -200.0f + 100 * i)));
...
/*NB this transparency check is a bit of a cheat: we're going for alpha value, but in reality, transparent game objects may have an alpha of 1, and what
would be checked against is some kind of tag. That's a bit involved for the sake of a tutorial, however! So the alpha will do.*/
...
{
	//reverse order sort! Note rbegin()/rend()
